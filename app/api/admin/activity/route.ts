...
export async function GET() {
  try {
    ...
    for (const p of (profilesRes.data ?? []) as ProfileRow[]) {
      const name = [p.first_name, p.last_name].filter(Boolean).join(' ') || p.email || p.id.slice(0, 8);
    ...
    type ExRow       = { user_id: string; name: string; calories_burned: number; duration: number; logged_at: string };
    type WeightRow   = { user_id: string; weight_kg: number; recorded_at: string };

    const events = [
      ...(foodRes.data ?? []).map((r: FoodRow) => ({
        type: 'food' as const,
        user: getName(r.user_id),
        label: `Logged ${Math.round(r.calories)} kcal (${r.meal_type})`,
        at: r.logged_at,
      })),
      ...(exerciseRes.data ?? []).map((r: ExRow) => ({
        type: 'exercise' as const,
        user: getName(r.user_id),
        label: `${r.name} — ${r.duration} min · ${r.calories_burned} kcal`,
        at: r.logged_at,
      })),
      ...(weightRes.data ?? []).map((r: WeightRow) => ({
        type: 'weight' as const,
        user: getName(r.user_id),
        label: `Logged weight: ${r.weight_kg} kg`,
        at: r.recorded_at,
      })),
    ]
